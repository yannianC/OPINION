        ...
        <section class="section auto-hedge-section">
          ...
          <div class="section-header-with-filter">
            ...
            <div class="trending-filter">
              <label>账号选择:</label>
              <select 
                v-model.number="hedgeMode.numberType" 
                class="filter-input" 
                :disabled="autoHedgeRunning"
                @change="saveHedgeSettings"
              >
          ...
          <div class="auto-hedge-controls">
            ...
            <label style="font-size: 14px; margin-left: 8px;">一个主题同时任务个数：</label>
              ...
              <button 
                ...
                :class="{ 'active': sortByYesDesc }"
                style="margin-left: 10px;"
                @click="sortByYesDesc = !sortByYesDesc"
              >
                {{ sortByYesDesc ? '▼ 已按YES数量排序' : '按YES数量排序' }}
              </button>
          </div>
          ...
          <div class="export-blacklist-controls" style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px; padding: 10px; background: #f5f5f5; border-radius: 8px;">
            ...
            <div style="display: flex; align-items: center; gap: 8px;">
                <label style="display: flex; align-items: center; gap: 4px; font-size: 14px; margin: 0 8px;color: #000;">
                  <input 
                    type="checkbox" 
                ...
                <input 
                  type="text" 
                  v-model="exportWhitelistInput" 
                  placeholder="只启用白名单中的ID (逗号分隔)" 
                  class="filter-input" 
                ...
                <div class="trending-header">
                  <div class="trending-name-row">
                    <span class="trending-name">
                      <span style="color: #fff; font-size: 12px; margin-right: 6px;">[{{ config.id }}]</span>{{ config.trending }}
                    </span>
                  ...
                  <div class="type3-task-section">
                    ...
                    <div v-if="config.orderbookData" class="type3-task-info">
                      <div class="task-status-row">
                        <span class="task-label">先挂方: {{ config.orderbookData.firstSide || '未知' }}</span>
                        <span class="task-status-badge" :class="config.orderbookData.reason ? 'status-error' : 'status-success'">
                          {{ config.orderbookData.reason ? '不符合条件' : '已更新' }}
                        </span>
                      </div>
                      <div v-if="config.orderbookData.reason" class="task-error-reason" style="color: #e74c3c; font-size: 12px; margin-top: 4px;">
                        {{ config.orderbookData.reason }}
                      </div>
                    ...
                    <div v-if="config.currentHedges && config.currentHedges.length > 0" class="hedge-info-list">
                      <div 
                        v-for="(hedge, index) in config.currentHedges.filter(h => {
                          // 模式1：只显示运行中的
                          if (!h.isMode2 && !h.isModeV4) {
                            return h.finalStatus === 'running'
                          }
                          // Type4：显示有closeLimitList或closeTasks的
                          if (h.isModeV4) {
                            return h.finalStatus === 'running' || 
                                   (h.closeTasks && h.closeTasks.length > 0) ||
                                   (h.secondTasks && h.secondTasks.length > 0) ||
                                   (h.closeLimitList && h.closeLimitList.length > 0) ||
                                   (h.secondLimitList && h.secondLimitList.length > 0)
                          }
                      ...
                      >
                        <div class="hedge-status-row">
                          ...
                          <span v-if="hedge.isModeV4" class="hedge-mode-badge">模式4</span>
                        </div>
                        
                        <!-- Type4：先挂方和后挂方任务列表展示方式 -->
                        <template v-if="hedge.isModeV4">
                          <!-- 先挂方任务列表 -->
                          <div v-if="(hedge.closeTasks && hedge.closeTasks.length > 0) || (hedge.closeLimitList && hedge.closeLimitList.length > 0)" class="hedge-task-section">
                            <div class="task-title">
                              先挂方 ({{ (hedge.closeTasks && hedge.closeTasks.length > 0) ? hedge.closeTasks.length : (hedge.closeLimitList ? hedge.closeLimitList.length : 0) }}个) 卖
                            </div>
                            <!-- 显示已提交的任务 -->
                            <template v-if="hedge.closeTasks && hedge.closeTasks.length > 0">
                              <div v-for="(task, taskIndex) in hedge.closeTasks" :key="taskIndex" class="hedge-task-item">
                            ...
                            <!-- 显示计划任务（还未提交） -->
                            <template v-else-if="hedge.closeLimitList && hedge.closeLimitList.length > 0">
                              <div v-for="(item, itemIndex) in hedge.closeLimitList" :key="itemIndex" class="hedge-task-item">
                                <div class="hedge-task-details-grid">
                                  <div class="hedge-detail-row">
                                    <span>任务ID:</span>
                                  ...
                                  <div class="hedge-detail-row">
                                    <span>电脑组:</span>
                                    <span>{{ item.group || getGroupNo(item.number) }}</span>
                                  </div>
                                  <div class="hedge-detail-row">
                                    <span>价格:</span>
                                    <span>{{ parseFloat(hedge.price) }}¢</span>
                                  </div>
                                  <div class="hedge-detail-row">
                                    <span>数量:</span>
                          ...
                          <!-- 后挂方任务列表 -->
                          <div v-if="(hedge.secondTasks && hedge.secondTasks.length > 0) || (hedge.secondLimitList && hedge.secondLimitList.length > 0)" class="hedge-task-section">
                            <div class="task-title">
                              后挂方 ({{ (hedge.secondTasks && hedge.secondTasks.length > 0) ? hedge.secondTasks.length : (hedge.secondLimitList ? hedge.secondLimitList.length : 0) }}个) 买
                            </div>
                            <!-- 显示已提交的任务 -->
                            <template v-if="hedge.secondTasks && hedge.secondTasks.length > 0">
                              <div v-for="(task, taskIndex) in hedge.secondTasks" :key="taskIndex" class="hedge-task-item">
                                <div class="hedge-task-details-grid">
                                  <div class="hedge-detail-row">
                                    <span>任务ID:</span>
                                    <span :class="getTaskStatusClass(task.status)">
                            ...
                            <!-- 显示计划任务（还未提交） -->
                            <template v-else-if="hedge.secondLimitList && hedge.secondLimitList.length > 0">
                              <div v-for="(item, itemIndex) in hedge.secondLimitList" :key="itemIndex" class="hedge-task-item">
                                <div class="hedge-task-details-grid">
                                  <div class="hedge-detail-row">
                                    <span>任务ID:</span>
                                    <span class="task-pending">待提交</span>
          ...
          <div v-else class="hedge-log-list">
            ...
            >
              <div class="log-header">
                ...
                <span v-if="log.isModeV4" class="log-mode-badge">模式4</span>
              ...
              <div class="log-details">
                ...
                <!-- 模式1：原有展示方式 -->
                <template v-if="!log.isMode2 && !log.isModeV4">
                  <div class="log-row">
                ...
                <template v-else-if="log.isModeV4">
                  <div v-if="(log.closeTasks && log.closeTasks.length > 0) || (log.closeLimitList && log.closeLimitList.length > 0)" class="log-row">
                    <span class="log-label">先挂方 ({{ (log.closeTasks && log.closeTasks.length > 0) ? log.closeTasks.length : (log.closeLimitList ? log.closeLimitList.length : 0) }}个) 卖:</span>
                    <div class="log-task-list">
                      <template v-if="log.closeTasks && log.closeTasks.length > 0">
                        <div v-for="(task, taskIndex) in log.closeTasks" :key="taskIndex" class="log-task-item">
                          浏览器{{ task.number }} | 任务{{ task.taskId || '-' }} | 数量{{ task.share }} | {{ getStatusText(task.status) }}
                      ...
                      <template v-else-if="log.closeLimitList && log.closeLimitList.length > 0">
                        <div v-for="(item, itemIndex) in log.closeLimitList" :key="itemIndex" class="log-task-item">
                          浏览器{{ item.number }} | 任务待提交 | 数量{{ item.share }} | 待提交
                        </div>
                      </template>
                    </div>
                  </div>
                  <div v-if="(log.secondTasks && log.secondTasks.length > 0) || (log.secondLimitList && log.secondLimitList.length > 0)" class="log-row">
                    <span class="log-label">后挂方 ({{ (log.secondTasks && log.secondTasks.length > 0) ? log.secondTasks.length : (log.secondLimitList ? log.secondLimitList.length : 0) }}个) 买:</span>
                    <div class="log-task-list">
                      <template v-if="log.secondTasks && log.secondTasks.length > 0">
                        <div v-for="(task, taskIndex) in log.secondTasks" :key="taskIndex" class="log-task-item">
                          浏览器{{ task.number }} | 任务{{ task.taskId || '-' }} | 数量{{ task.share }} | {{ getStatusText(task.status) }}
                          <span v-if="task.msg" class="task-msg">| {{ formatTaskMsg(task.msg) }}</span>
                        </div>
                      </template>
                      <template v-else-if="log.secondLimitList && log.secondLimitList.length > 0">
                        <div v-for="(item, itemIndex) in log.secondLimitList" :key="itemIndex" class="log-task-item">
                          浏览器{{ item.number }} | 任务待提交 | 数量{{ item.share }} | 待提交
                        </div>
                      </template>
                    </div>
          ...
          <div v-else class="all-hedge-log-list">
            ...
            >
              <div class="compact-log-main">
                ...
                <span v-if="log.isModeV4" class="compact-log-mode-badge">模式4</span>
                <span class="compact-log-info">
                  ...
                  <template v-else-if="log.isModeV4">
                    价格:{{ log.price }} | 
                    先挂方:{{ (log.closeTasks && log.closeTasks.length > 0) ? log.closeTasks.length : (log.closeLimitList ? log.closeLimitList.length : 0) }}个 | 
                    后挂方:{{ (log.secondTasks && log.secondTasks.length > 0) ? log.secondTasks.length : (log.secondLimitList ? log.secondLimitList.length : 0) }}个
                  </template>
                </span>
                      ...
                      <template v-if="log.yesTasks && log.yesTasks.length > 0">
                        ...
                        <template v-for="(task, taskIndex) in log.yesTasks" :key="taskIndex">
                          ...
                          <button 
                            v-if="task.number"
                            class="btn-view-log" 
                            @click="openBroLogDialog(task.number)"
                            style="margin-left: 8px; padding: 2px 8px; font-size: 12px;"
                          >
                            查看日志
                          </button>
...
const sortByYesDesc = ref(false)  // 是否按yes数量倒序排序
...
const hedgeMode = reactive({
  ...
  // 订单薄筛选设置
  minOrderbookDepth: 3,  // 订单薄至少组数
  maxPriceDiff: 15,  // 最大价差
  ...
  enableDepthDiffParams01: false,    // 深度差0.1的开关
  // 各深度区间价格波动配置（百分比）
...
/**
 * 监听测试模式开关变化，当关闭时清除所有配置的请求标记
 */
watch(
  () => testModeSingleGroup.value,
  (newValue, oldValue) => {
    // 当测试模式从开启变为关闭时，清除所有配置的请求标记
    if (oldValue === true && newValue === false) {
      for (const config of activeConfigs.value) {
        config.hasRequestedGroup = false
      }
      console.log('测试模式已关闭，已清除所有配置的请求标记')
    }
  }
)
...
const filteredActiveConfigs = computed(() => {
  ...
  // 排序逻辑：如果开启了按yes数量倒序排序
  if (sortByYesDesc.value) {
    // 创建新数组进行排序，避免修改原数组顺序
    result = [...result].sort((a, b) => {
      const yesA = positionDataMap.value.get(String(a.id))?.yesSum || 0
      const yesB = positionDataMap.value.get(String(b.id))?.yesSum || 0
      return yesB - yesA // 降序：大到小
    })
  }
...
const stopAutoHedge = () => {
  ...
  for (const config of activeConfigs.value) {
    ...
    // 清除测试模式标记，以便下次开启时可以重新请求
    if (testModeSingleGroup.value) {
      config.hasRequestedGroup = false
    }
    config.retryCount = 0
...
const executeAllTopics = async () => {
  ...
  debugLog(`开始执行所有 ${validConfigs.length} 个主题（不分批模式，按列表顺序执行）`)
        ...
        if (runningHedges.length > 0) {
          ...
          if (config.lastGroupFinishTime && hedgeTaskInterval.value > 0) {
            ...
            if (testModeSingleGroup.value && config.hasRequestedGroup) {
              debugLog(`配置 ${config.id} - 测试模式已开启，已请求过一组任务，停止请求新任务`)
              // 跳过，但会在循环末尾等待1秒
            } else {
              // 可以继续处理
              await processConfigOrderbook(config, canStartNewHedge)
  ...
  try {
    ...
    if (canStartNewHedge) {
      ...
      if (hedgeCheck.canHedge) {
        ...
        debugLog(`配置 ${config.id} - 不满足对冲条件: ${hedgeCheck.reason}`)
...
const fetchOrderbookBasic = async (config, isClose) => {
  try {
    ...
    // 获取订单薄数据成功后，调用 snapDepth 接口
    try {
      // 调用 YES 的 snapDepth
      await axios.post('https://sg.bicoin.com.cn/99l/hedge/snapDepth', {
        ...
        console.warn(`配置 ${config.id} - 调用 snapDepth (YES) 失败:`, err)
      })
      ...
      await axios.post('https://sg.bicoin.com.cn/99l/hedge/snapDepth', {
        ...
        console.warn(`配置 ${config.id} - 调用 snapDepth (NO) 失败:`, err)
      })
    } catch (snapDepthError) {
      // snapDepth 调用失败不影响主流程，只记录日志
      console.warn(`配置 ${config.id} - snapDepth 调用异常:`, snapDepthError)
    }
...
const parseOrderbookData = async (config, isClose) => {
  ...
  try {
    // 获取yes和no的订单薄数据
    // 如果获取失败，fetchOrderbook应该会抛出异常，被catch捕获
    const [yesOrderbook, noOrderbook] = await Promise.all([
      fetchOrderbook(config.trendingPart1),
      fetchOrderbook(config.trendingPart2)
    ])
    ...
    if (yesBids.length < minDepth || yesAsks.length < minDepth || 
       ...
       // 标记失败但不立即返回，尝试继续获取价格用于显示
       if (!result.failReason) result.failReason = `订单薄数据不足${minDepth}组`
    }
    ...
    // 转换为百分比格式
    const yesBidPrice = parseFloat(yesBids[0].price) * 100
    const yesAskPrice = parseFloat(yesAsks[0].price) * 100
    const noBidPrice = parseFloat(noBids[0].price) * 100
    const noAskPrice = parseFloat(noAsks[0].price) * 100
    
    const yesBidDepth = parseFloat(yesBids[0].size)
    ...
    result.firstSide = getNextFirstSide(config.id, yesBidPrice, noBidPrice, yesAskPrice, noAskPrice, isClose)
    
    const firstBids = result.firstSide === 'YES' ? yesBids : noBids
    const firstAsks = result.firstSide === 'YES' ? yesAsks : noAsks
    
    // 获取先挂方的买一价和卖一价
    if (result.firstSide === 'YES') {
      result.price1 = yesBidPrice
      result.price2 = yesAskPrice
      result.depth1 = yesBidDepth
      result.depth2 = yesAskDepth
    } else {
      result.price1 = noBidPrice
      result.price2 = noAskPrice
      result.depth1 = noBidDepth
      result.depth2 = noAskDepth
    }
    
    // 基础计算
    result.diff = result.price2 - result.price1
    console.log(`深度差: ${result.diff.toFixed(2)}`)
    result.minPrice = Math.min(result.price1, result.price2)
    result.maxPrice = Math.max(result.price1, result.price2)

    // === 深度检查 ===
    // 即使前面 depth check 失败，也计算一下累计深度用于返回
    const depthCount = hedgeMode.minOrderbookDepth
    // 防止 slice 出错
    const safeFirstBids = firstBids.slice(0, Math.min(firstBids.length, depthCount))
    const safeFirstAsks = firstAsks.slice(0, Math.min(firstAsks.length, depthCount))
    
    result.topNBidsDepth = safeFirstBids.reduce((sum, bid) => sum + parseFloat(bid.size), 0)
    result.topNAsksDepth = safeFirstAsks.reduce((sum, ask) => sum + parseFloat(ask.size), 0)
    
    console.log(`先挂方 ${result.firstSide} - 买1-${depthCount}深度累计: ${result.topNBidsDepth.toFixed(2)}, 卖1-${depthCount}深度累计: ${result.topNAsksDepth.toFixed(2)}`)
    
    const minTotalDepth = hedgeMode.minTotalDepth
    if (!result.failReason && (result.topNBidsDepth < minTotalDepth || result.topNAsksDepth < minTotalDepth)) {
      result.failReason = `深度不足：买1-${depthCount}累计=${result.topNBidsDepth.toFixed(2)}, 卖1-${depthCount}累计=${result.topNAsksDepth.toFixed(2)}, 要求>=${minTotalDepth}`
    }
    
    // === 价差检查 ===
    if (!result.failReason) {
        const maxPriceDiff = hedgeMode.maxPriceDiff
        const depthIndex = hedgeMode.minOrderbookDepth - 1
        
        // 确保有足够数据进行价差检查
        if (firstBids.length > depthIndex && firstAsks.length > depthIndex) {
            if (isClose) {
                ...
                if (bidsPriceDiff >= maxPriceDiff) {
                    result.failReason = `买1-买${hedgeMode.minOrderbookDepth}价差过大: ${bidsPriceDiff.toFixed(2)} >= ${maxPriceDiff}`
                }
            } else {
                ...
                if (asksPriceDiff >= maxPriceDiff) {
                    result.failReason = `卖1-卖${hedgeMode.minOrderbookDepth}价差过大: ${asksPriceDiff.toFixed(2)} >= ${maxPriceDiff}`
                }
            }
        } else {
             // 如果数据不足，已经在前面设置了failReason，这里忽略或者是次要错误
        }
    }
    
    // === 深度差阈值与计算逻辑 ===
    // 注意：价格区间检查移到深度差计算完成后，基于 finalPrice 进行检查
    const threshold1 = hedgeMode.depthThreshold1
    const threshold2 = hedgeMode.depthThreshold2
    const threshold3 = hedgeMode.depthThreshold3
    ...
    if (result.diff > threshold1 - 0.00001) {
        if (!hedgeMode.enableDepthDiffParamsGt15) {
            if (!result.failReason) result.failReason = `深度差>${threshold1}时，开关未开启（当前: ${result.diff.toFixed(2)}）`
        }
        result.depthDiffRange = 'gt15'
        if (!result.failReason) {
        ...
        if (!hedgeMode.enableDepthDiffParams2To15) {
            if (!result.failReason) result.failReason = `深度差在${threshold2}-${threshold1}区间时，开关未开启（当前: ${result.diff.toFixed(2)}）`
        }
        result.depthDiffRange = '2to15'
        if (!result.failReason) {
            ...
            console.log(`深度差 ${threshold2}-${threshold1} - 计算最终价格: ${result.finalPrice.toFixed(2)}`)
        }
    } else if (result.diff >= threshold3 - 0.00001) {
        if (!hedgeMode.enableDepthDiffParams02To2) {
            if (!result.failReason) result.failReason = `深度差在${threshold3}-${threshold2}区间时，开关未开启（当前: ${result.diff.toFixed(2)}）`
        }
        result.depthDiffRange = '02to2'
        if (!result.failReason) {
            const minVolatility = hedgeMode.priceVolatility02To2Min
            const maxVolatility = hedgeMode.priceVolatility02To2Max
            const adjustment = calculatePriceAdjustment(result.diff, minVolatility, maxVolatility)
            result.finalPrice = isClose ? result.price2 - adjustment : result.price1 + adjustment
            console.log(`深度差 ${threshold3}-${threshold2} - 计算最终价格: ${result.finalPrice.toFixed(2)}`)
        }
    } else if (Math.abs(result.diff - 0.1) < 0.01) {
        if (!hedgeMode.enableDepthDiffParams01) {
            if (!result.failReason) result.failReason = `深度差0.1时，开关未开启（当前: ${result.diff.toFixed(2)}）`
        }
        result.depthDiffRange = '01'
        if (!result.failReason) {
            const maxEatValue = hedgeMode.maxEatValue01
            const maxDepthValue = hedgeMode.maxDepth
            
            if (isClose) {
                // 平仓
                const askValue = result.price2 * result.depth2 / 100
                if (askValue < maxDepthValue) {
                     result.finalPrice = result.price2
                     console.log(`深度差0.1平仓，卖一价值<最大深度，使用卖一价: ${result.finalPrice.toFixed(2)}`)
                } else {
                     const bidValue = result.price1 * result.depth1 / 100
                     if (bidValue >= maxEatValue) {
                         result.failReason = `深度差0.1平仓，卖一价值${askValue.toFixed(2)}>=${maxDepthValue}且买一价值${bidValue.toFixed(2)}>=${maxEatValue}`
                     } else {
                         result.finalPrice = result.price1
                         result.firstShareReduction = (maxEatValue / result.price1) * 100
                         console.log(`深度差0.1平仓，使用买一价: ${result.finalPrice.toFixed(2)}, 减少: ${result.firstShareReduction.toFixed(2)}`)
                     }
                }
            } else {
                // 开仓
                // 计算 买一价深度*买一价/100
                const bidValue = result.price1 * result.depth1 / 100
                if (bidValue < maxDepthValue) {
                    result.finalPrice = result.price1
                    console.log(`深度差0.1开仓，买一价值<最大深度，使用买一价: ${result.finalPrice.toFixed(2)}`)
                } else {
                    const askValue = result.price2 * result.depth2 / 100
                    if (askValue >= maxEatValue) {
                        result.failReason = `深度差0.1开仓，买一价值${bidValue.toFixed(2)}>=${maxDepthValue}且卖一价值${askValue.toFixed(2)}>=${maxEatValue}`
                    } else {
                        result.finalPrice = result.price2
                        result.firstShareReduction = (maxEatValue / result.price2) * 100
                        console.log(`深度差0.1开仓，使用卖一价: ${result.finalPrice.toFixed(2)}, 减少: ${result.firstShareReduction.toFixed(2)}`)
                    }
                }
            }
        }
    } else {
        if (!result.failReason) result.failReason = `深度差过小: ${result.diff.toFixed(2)} < ${threshold3}`
    }
    
    // === 先挂方价格区间检查（深度差判断通过后，基于 finalPrice 检查）===
    if (!result.failReason && result.finalPrice !== null) {
        const priceMin = hedgeMode.priceRangeMin
        const priceMax = hedgeMode.priceRangeMax
        const mode = isClose ? '平仓' : '开仓'
        
        console.log(`${mode}模式 - 先挂方价格: ${result.finalPrice.toFixed(2)}, 价格区间要求: ${priceMin}-${priceMax}`)
        
        if (result.finalPrice < priceMin) {
            result.failReason = `${mode}模式，先挂方价格 ${result.finalPrice.toFixed(2)} 小于最小价格 ${priceMin}`
        }
        
        if (result.finalPrice > priceMax) {
            result.failReason = `${mode}模式，先挂方价格 ${result.finalPrice.toFixed(2)} 大于最大价格 ${priceMax}`
        }
    }
    
    // 如果没有任何 failure，则 valid = true
    if (!result.failReason) {
        result.isValid = true
    }
    ...
    console.error(`配置 ${config.id} - 解析订单薄数据出错:`, error)
    if (!result.failReason) {
       result.failReason = error.message || '系统错误'
    }
    return result
  }
}
...
const checkOrderbookHedgeCondition = (priceInfo) => {
  ...
  // 检查 parseOrderbookData 返回的 isValid 和 failReason
  if (priceInfo.failReason) {
    return { canHedge: false, reason: priceInfo.failReason }
  }
  if (priceInfo.isValid === false) {
    return { canHedge: false, reason: '订单薄数据校验未通过' }
  }
...
const executeHedgeFromOrderbook = async (config, priceInfo) => {
  try {
    ...
    debugLog(`配置 ${config.id} - 需要执行 ${availableSlots} 个对冲任务（最大: ${taskCount}, 当前运行: ${runningHedges.length}）`)
      ...
      try {
        ...
        if (orderPriceNum < 50) {
          ...
          if (hedgeMode.enableMove) {
            ...
            shouldExecute = true
            debugLog(`配置 ${config.id} - 价格 ${orderPriceNum} < 50，且挪仓开关已开启，使用 model4Move 接口`)
          } else {
            debugLog(`配置 ${config.id} - 价格 ${orderPriceNum} < 50，但挪仓开关未开启，跳过`)
            continue
          }
        } else {
          ...
          if (hedgeMode.enableClose) {
            ...
            shouldExecute = true
            debugLog(`配置 ${config.id} - 价格 ${orderPriceNum} >= 50，且平仓开关已开启，使用 model5Close 接口`)
          } else {
            debugLog(`配置 ${config.id} - 价格 ${orderPriceNum} >= 50，但平仓开关未开启，跳过`)
            continue
          }
        }
        ...
        const canTradeBy901To927 = await checkTask901To927Condition()
        if (!canTradeBy901To927) {
          debugLog(`配置 ${config.id} - 901-927任务数不满足条件，跳过本次对冲任务请求`)
          continue
        }
        ...
        if (response.data && response.data.data) {
          const responseData = response.data.data
          debugLog(`配置 ${config.id} - 获取对冲数据成功 (任务 ${i + 1}/${availableSlots}):`, responseData)
          
          // 保存missionId用于下次请求（如果存在）
          if (responseData.missionId) {
            config.lastMissionId = responseData.missionId
          }
          
          // 新接口同时返回先挂方列表和后挂方信息
          if (responseData.closeLimitList && Array.isArray(responseData.closeLimitList) && 
              responseData.takerNumberInfo && typeof responseData.takerNumberInfo === 'object') {
            await executeHedgeTaskV4(config, {
              closeLimitList: responseData.closeLimitList,
              takerNumberInfo: responseData.takerNumberInfo,
              currentPrice: orderPrice,
            ...
            // 测试模式：标记已请求过一组任务
            if (testModeSingleGroup.value && i === 0) {
              config.hasRequestedGroup = true
              debugLog(`配置 ${config.id} - 测试模式：已请求到一组任务，标记为已请求，后续将不再请求新任务`)
            }
          } else {
            console.warn(`配置 ${config.id} - closeLimitList 或 takerNumberInfo 为空或格式不正确`)
          }
...
const checkAndReplaceTopics = async () => {
  ...
  try {
    ...
    const topicsToReplace = activeConfigs.value.filter(config => {
      ...
      // 如果有正在运行的对冲任务，不替换
      if (runningHedges.length > 0) {
        debugLog(`配置 ${config.id} - 有正在运行的对冲任务，暂不替换`)
        return false
      }
...
const loadHedgeSettings = () => {
  try {
    ...
    if (settings.numberType !== undefined) {
      hedgeMode.numberType = settings.numberType
    }
    ...
    if (settings.closeAmtSumMaxStart !== undefined) {
      hedgeMode.closeAmtSumMaxStart = settings.closeAmtSumMaxStart
    }
    ...
    // 白名单设置
    if (settings.enableWhitelist !== undefined) {
      enableWhitelist.value = settings.enableWhitelist
    }
...
const submitSecondHedgeTask = async (config, hedgeRecord) => {
  ...
  try {
    ...
    // 任务二的价格 = 100 - 任务一的价格
    const secondPrice = (100 - parseFloat(hedgeRecord.price)).toFixed(2)
...
/**
 * 执行对冲任务（Type4 - 使用closeLimitList和takerNumberInfo，同时提交先挂方和后挂方）
 */
const executeHedgeTaskV4 = async (config, hedgeData) => {
  ...
  const hedgeRecord = {
    ...
    duration: null,
    finalStatus: 'running',  // running, success, failed
    isModeV4: true,  // 标记为Type4
    missionId: hedgeData.missionId,  // 保存missionId用于后续请求
    closeTasks: [],  // 先挂方任务列表 [{number, share, taskId, status, groupNo, price}]
    secondTasks: [],  // 后挂方任务列表 [{number, share, taskId, status, groupNo, price}]
    closeLimitList: closeLimitList,  // 先挂方原始列表
    takerNumberInfo: takerNumberInfo,  // 后挂方原始信息（用于显示）
    allTaskIds: [],  // 所有任务ID的数组
    blackGroupList: [],  // 失败的电脑组列表
    blackNumberList: []  // 失败的浏览器编号列表
  }
  ...
  console.log(`开始对冲（Type4） ${config.id}:`, hedgeRecord)
  
  try {
    if (closeLimitList.length === 0) {
      console.warn(`配置 ${config.id} - closeLimitList为空，无法执行对冲`)
      hedgeRecord.finalStatus = 'failed'
      hedgeRecord.errorMsg = 'closeLimitList为空'
      finishHedge(config, hedgeRecord)
      return
    }
    
    if (!takerNumberInfo || typeof takerNumberInfo !== 'object') {
      console.warn(`配置 ${config.id} - takerNumberInfo为空或格式不正确，无法执行对冲`)
      hedgeRecord.finalStatus = 'failed'
      hedgeRecord.errorMsg = 'takerNumberInfo为空或格式不正确'
      finishHedge(config, hedgeRecord)
      return
    }
    
    const firstPsSide = firstSide === 'YES' ? 1 : 2
    const firstTaskPrice = firstPrice
    
    // 根据先挂方价格计算后挂方参数
    let secondPsSide, secondTaskPrice, secondSide
    if (firstPrice < 50) {
      // 价格小于50：后挂方YES/NO跟先挂方一样，买入开仓，价格一样
      secondPsSide = firstSide === 'YES' ? 1 : 2  // 跟先挂方一样
      ...
      secondPsSide = firstSide === 'YES' ? 2 : 1  // 跟先挂方相反（YES变NO，NO变YES）
    ...
    try {
      ...
      const taskData = {
        ...
        tp14: Math.round(cancelOrderDelaySeconds),  // 取消挂单延迟时间（秒）
    ...
    for (const item of closeLimitList) {
      try {
        const browserNo = item.number
        const share = floorToTwoDecimals(item.share)
        // 使用item.group，如果没有则使用browserToGroupMap
        let groupNo = '1'
        if (item.group !== undefined && item.group !== null && item.group !== '') {
          groupNo = String(item.group)
        } else {
        ...
        if (response.data && response.data.data) {
          ...
          if (taskId === undefined || taskId === null || typeof taskId === 'object') {
            console.error(`Type4 - 提交先挂方任务失败: 无效的任务ID`, { taskData, taskId })
            continue
          }
          
          taskId = String(Number(taskId))
          console.log(`Type4 - 先挂方任务提交成功，浏览器: ${browserNo}, 任务ID: ${taskId}`)
          ...
          hedgeRecord.closeTasks.push(taskInfo)
          hedgeRecord.allTaskIds.push(taskId)
          firstSideSuccessCount++
        } else if (response.data && response.data.msg) {
          console.error(`Type4 - 提交先挂方任务失败（浏览器: ${browserNo}）:`, response.data.msg)
        }
      } catch (error) {
        console.error(`Type4 - 提交先挂方任务失败（浏览器: ${item.number}）:`, error)
      }
    }
    
    if (firstSideSuccessCount === 0) {
      console.error(`Type4 - 所有先挂方任务提交失败`)
      hedgeRecord.finalStatus = 'failed'
      hedgeRecord.errorMsg = '所有先挂方任务提交失败'
      finishHedge(config, hedgeRecord)
      return
    }
    ...
    console.error('Type4 - 执行对冲任务失败:', error)
    hedgeRecord.finalStatus = 'failed'
    finishHedge(config, hedgeRecord)
  }
}
...
/**
 * 监控对冲状态（Type4 - 同时监控先挂方和后挂方所有任务）
 */
const monitorHedgeStatusV4 = (config, hedgeRecord) => {
  const startTime = new Date(hedgeRecord.startTime)
  let phase0Completed = false  // 阶段0是否完成（等待状态18）
  ...
  const checkStatus = async () => {
    ...
    if (!phase0Completed) {
      ...
      if (isPhase0Completed) {
        ...
        // 如果后挂方任务成功（状态18）且先挂方有状态18的任务
        if (hasSecondTaskStatus18 && hasCloseTaskStatus18) {
          console.log(`[Type4] 阶段0成功：后挂方任务状态18，先挂方有状态18的任务`)
          
          // 计算并更新吃单方（后挂方）的amt
          // 1. 获取后挂方任务的amt
          const takerTask = secondTasks.find(t => t.status === 18)
          if (takerTask) {
            ...
            // 3. 计算新的amt（向上取整保留2位小数）
            const newAmt = Math.ceil((takerAmt - failedMakerAmtSum) * 100) / 100
            console.log(`[Type4] 吃单方原amt: ${takerAmt}, 先挂方失败任务amt总和: ${failedMakerAmtSum}, 新amt: ${newAmt}`)
            
            // 4. 如果有失败的先挂方任务，调用接口更新后挂方任务的amt（重试3次）
            if (failedMakerAmtSum > 0 && newAmt > 0) {
              let updateAmtSuccess = false
              const maxAmtRetries = 3
              
              for (let retry = 0; retry < maxAmtRetries; retry++) {
                try {
                  const response = await axios.post('https://sg.bicoin.com.cn/99l/mission/updateMissionTp', {
                    id: takerTask.taskId,
                    amt: newAmt
                  })
                  
                  if (response.data && response.data.code === 0) {
                    console.log(`[Type4] 更新吃单方任务 ${takerTask.taskId} amt为 ${newAmt} 成功`)
                    // 更新本地记录
                    const takerIndex = hedgeRecord.secondTasks.findIndex(t => t.taskId === takerTask.taskId)
                    if (takerIndex !== -1) {
                      hedgeRecord.secondTasks[takerIndex] = { ...hedgeRecord.secondTasks[takerIndex], share: newAmt }
                    }
                    updateAmtSuccess = true
                    break
                  } else {
                    console.log(`[Type4] 更新吃单方任务amt失败 (重试 ${retry + 1}/${maxAmtRetries}): ${response.data?.msg || '未知错误'}`)
                  }
                } catch (error) {
                  console.error(`[Type4] 更新吃单方任务amt异常 (重试 ${retry + 1}/${maxAmtRetries}):`, error)
                }
                
                if (retry < maxAmtRetries - 1) {
                  await new Promise(resolve => setTimeout(resolve, 1000))
                }
              }
              
              if (!updateAmtSuccess) {
                console.log(`[Type4] 更新吃单方任务amt失败，已重试${maxAmtRetries}次，将所有任务状态改为3`)
                await setAllTasksToFailed()
                hedgeRecord.finalStatus = 'failed'
                hedgeRecord.errorMsg = '更新吃单方amt失败'
                console.log(`[Type4] 整组任务结束，最终状态: ${hedgeRecord.finalStatus}`)
                finishHedge(config, hedgeRecord)
                return
              }
            } else if (newAmt <= 0) {
              console.log(`[Type4] 新amt ${newAmt} <= 0，将所有任务状态改为3`)
              await setAllTasksToFailed()
              hedgeRecord.finalStatus = 'failed'
              hedgeRecord.errorMsg = '计算后amt无效'
              console.log(`[Type4] 整组任务结束，最终状态: ${hedgeRecord.finalStatus}`)
              finishHedge(config, hedgeRecord)
              return
            }
          ...
          console.log(`[Type4] 开始将状态18的任务改为19`)
          
          // 将所有状态18的任务改为19
          for (let i = 0; i < hedgeRecord.closeTasks.length; i++) {
            const task = hedgeRecord.closeTasks[i]
            if (task.taskId && task.status === 18) {
              console.log(`[Type4] 将先挂方任务 ${task.taskId} 状态从18改为19`)
              hedgeRecord.closeTasks[i] = { ...hedgeRecord.closeTasks[i], status: 19 }
              await updateTaskStatus(task.taskId, 19)
            }
          }
          ...
          for (let i = 0; i < hedgeRecord.secondTasks.length; i++) {
            const task = hedgeRecord.secondTasks[i]
            if (task.taskId && task.status === 18) {
              console.log(`[Type4] 将后挂方任务 ${task.taskId} 状态从18改为19`)
              hedgeRecord.secondTasks[i] = { ...hedgeRecord.secondTasks[i], status: 19 }
              await updateTaskStatus(task.taskId, 19)
            }
          }
          
          phase0Completed = true
          phase1StartTime = Date.now()  // 阶段1开始计时
          
          // 初始化阶段1的超时时间（2分钟）
          allCurrentTasks.forEach(task => {
            if (task.taskId) {
              taskTimeoutMap.set(task.taskId, phase1StartTime + 2 * 60 * 1000)  // 阶段1：2分钟超时
            }
          })
          
          console.log(`[Type4] 阶段0完成，进入阶段1，重新开始计时（2分钟超时）`)
        } else {
          // 阶段0完成但主任务不成功
          console.log(`[Type4] 阶段0：主任务未成功，将所有任务状态改为 3，整组任务结束`)
          await setAllTasksToFailed()
          hedgeRecord.finalStatus = 'failed'
          hedgeRecord.errorMsg = '阶段0主任务未成功'
          console.log(`[Type4] 整组任务结束，最终状态: ${hedgeRecord.finalStatus}`)
          finishHedge(config, hedgeRecord)
          return
        }
        ...
        // 阶段0还没完成，显示进度并继续监控
        const runningTasks = allCurrentTasks.filter(t => t.status === 9 || t.status === 0 || t.status === 19)
        const completedTasks = allCurrentTasks.filter(t => t.status === 18 || t.status === 3)
        console.log(`[Type4] 阶段0进度: ${completedTasks.length}/${allCurrentTasks.length} 完成，${runningTasks.length} 个运行中，已用时 ${elapsed.toFixed(1)} 分钟`)
    ...
    // ========== 阶段1：等待所有任务状态为 3 或 20 或超时（2分钟） ==========
    
    // 修复状态竞争：检查是否有任务被脚本覆盖回状态18，如果是则重新改为19
    for (let i = 0; i < hedgeRecord.closeTasks.length; i++) {
      const task = hedgeRecord.closeTasks[i]
      if (task.taskId && task.status === 18) {
        console.log(`[Type4] 阶段1：检测到先挂方任务 ${task.taskId} 状态被覆盖回18，重新改为19`)
        hedgeRecord.closeTasks[i] = { ...hedgeRecord.closeTasks[i], status: 19 }
        await updateTaskStatus(task.taskId, 19)
      }
    }
    ...
    for (let i = 0; i < hedgeRecord.secondTasks.length; i++) {
      const task = hedgeRecord.secondTasks[i]
      if (task.taskId && task.status === 18) {
        console.log(`[Type4] 阶段1：检测到后挂方任务 ${task.taskId} 状态被覆盖回18，重新改为19`)
        hedgeRecord.secondTasks[i] = { ...hedgeRecord.secondTasks[i], status: 19 }
        await updateTaskStatus(task.taskId, 19)
      }
    }
    ...
    if (isPhase1Completed && !phase1Completed) {
      ...
      if (hasSecondTaskStatus20) {
        ...
        if (!hasCloseTaskStatus20) {
          console.log(`[Type4] 阶段1：后挂方状态是 20，但先挂方没有状态是 20 的任务，将所有任务状态改为 3，整组任务结束`)
          await setAllTasksToFailed()
          hedgeRecord.finalStatus = 'failed'
        ...
        if (!phase2InProgress) {
          ...
          console.log(`[Type4] 开始阶段2：调用 updateTrendingTime`)
            ...
            if (success) {
              ...
              for (let i = 0; i < hedgeRecord.closeTasks.length; i++) {
                ...
                if (task.status === 20 && task.taskId) {
                  ...
                  hedgeRecord.closeTasks[i] = { ...task, status: 31 }
                  await updateTaskStatus(task.taskId, 31)
                }
              }
              
              // 等待 10 秒后，将后挂方的状态改为 31
              ...
              for (let i = 0; i < hedgeRecord.secondTasks.length; i++) {
                ...
                if (task.status === 20 && task.taskId) {
                  ...
                  hedgeRecord.secondTasks[i] = { ...task, status: 31 }
                  await updateTaskStatus(task.taskId, 31)
                }
              }
              
              console.log(`[Type4] 阶段2完成，所有任务状态已更新为 31`)
              // 阶段2完成，整组任务成功结束
              hedgeRecord.finalStatus = 'success'
              console.log(`[Type4] 整组任务结束，最终状态: ${hedgeRecord.finalStatus}`)
              finishHedge(config, hedgeRecord)
    ...
    if (!phase1Completed) {
      ...
      const phase1Elapsed = phase1StartTime ? (now.getTime() - phase1StartTime) / 1000 / 60 : 0
      const runningTasks = allCurrentTasks.filter(t => t.status === 9 || t.status === 0 || t.status === 19)
      const completedTasks = allCurrentTasks.filter(t => t.status === 20 || t.status === 2 || t.status === 3)
      console.log(`[Type4] 阶段1进度: ${completedTasks.length}/${allCurrentTasks.length} 完成，${runningTasks.length} 个运行中，阶段1用时 ${phase1Elapsed.toFixed(1)} 分钟`)
...
/**
 * 格式化任务消息（支持JSON格式的Type 1、Type 5、Type 6消息）
 */
/**
 * 格式化任务消息显示
...
const formatTaskMsg = (msg) => {
  ...
  // 如果是字符串，尝试去除首尾空格
  const trimmedMsg = typeof msg === 'string' ? msg.trim() : msg
  
  // 尝试解析JSON格式的消息
  try {
    const data = JSON.parse(trimmedMsg)
    ...
    if (data.type === 'TYPE1_SUCCESS') {
      ...
      return result
    } else if (data.type === 'TYPE1_PARTIAL') {
      // Type 1 部分成交：有挂单
      let result = `⚠️ 部分成交`
      ...
      return result
    } else if (data.type === 'TYPE5_SUCCESS') {
      // Type 5 成功：全部成交
      let result = `✅ 全部成交`
      ...
      return result
    } else if (data.type === 'TYPE6_SUCCESS') {
      // Type 6 成功：全部成交
      let result = `✅ 挂单成功`
      ...
      return result
    } else if (data.type === 'TYPE6_PARTIAL') {
      // Type 6 部分成交：有挂单
      let result = `✅ 挂单成功`
    ...
    // 不是JSON格式，返回原始消息
    // console.warn('formatTaskMsg: JSON解析失败', e, '消息内容:', msg)
  }
  
  // 返回原始消息（如果JSON解析失败或type不匹配）
